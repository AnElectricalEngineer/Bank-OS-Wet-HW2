...
def generate_test():
    ...
    for i in range(atm_num): #run for every atm
        filename = "atm_" + str(i + 1) + ".txt"
        filepath = os.path.join(here, test_dir, filename)
        cmd_string = cmd_string + " " + subdir + "/" + test_dir + "/atm_" + str(i + 1) + ".txt" 
    ...
    for acc, passw in zip(accounts, passwords):
        cmd_fd.write("account: " + acc + ", password: " + passw + "\n")
    cmd_fd.close()
